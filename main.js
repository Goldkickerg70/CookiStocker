/* CookiStocker — fixed build
   - Guards CCSE usage & schedules safely
   - Defines ReplaceGameMenu before any call sites
   - Removes duplicate calcCommission definition
   - Hardens timers/hooks, load/save, and options syncing
   - Keeps all original behavior/options
*/
...
var stockerMinBrokers = 72;                   // Default 72 → ~0.5% commission
var stockerCookiesThreshold = 0.05;           // Fraction of banked cookies allowed per buy
var stockerAutoBuyMinimumBrokers = true;      // Buy up to minimum as soon as possible
var stockerAutoBuyAdditionalBrokers = true;   // Then buy additional as possible
var stockerExponential = true;                // Warehouse multiplier tied to CPS high
var stockerExponentialPower = 1.0;            // Exponent for exponential warehouses
var stockerTransactionNotifications = true;   // In-game notifications on buy/sell
var stockerActivityReport = false;            // Periodic reports
var stockerActivityReportFrequency = 1000 * 60 * 60; // 1 hour
var stockerFastNotifications = false;         // Make notifications fade after ~6s
var stockerConsoleAnnouncements = false;      // Log market updates to console
var stockerResourcesWarning = true;           // Show warning banner if auto-trade can’t run
var stockerAdditionalTradingStats = true;     // Show extra stats block
var stockerLoopFrequency = 1000 * 30;         // Logic loop cadence (ms)
...
if (typeof CCSE === 'undefined') {
  // Load CCSE in Steam/HTML builds
...
CookiStocker.version = '3.0.3-fix';
CookiStocker.GameVersion = '2.053';
CookiStocker.build = 'Mon 2025-11-10 21:05:00';
...
// NOTE: define ReplaceGameMenu **before** any code that could call it
CookiStocker.ReplaceGameMenu = function() {
  if (typeof CCSE === 'undefined' || !CCSE || !CCSE.AppendCollapsibleOptionsMenu || !CCSE.AppendStatsVersionNumber) {
    // CCSE not ready; caller will retry later
    throw new Error('CCSE not ready');
  }
  ...
  Game.customStatsMenu.push(function() {
    ...
    CCSE.AppendStatsGeneral(
      '<div class="listing"><b>Stock Market has earned you :</b><div class="price plain"> $'
      + Beautify(p) + ' (' + Game.tinyCookie() + Beautify(p * Game.cookiesPsRawHighest)
      + ' cookies)</div></div>'
    );
  });
};

// Helper shorthands used throughout
...
// === Anchored scheduling each market tick ===
...
CookiStocker.ensureReportTimer = function() {
  if (Game.OnAscend) {
    if (CookiStocker.reportTimer) clearInterval(CookiStocker.reportTimer);
    CookiStocker.reportTimer = 0;
  }
...
CookiStocker.getMenuString = function() {
  ...
  var cbWarehouseExponent =
      "stockerExponentialPower = (l('exponentSlider').value);"
  ...
  var cbMinBrokers =
      "stockerMinBrokers = Math.round(l('minBrokersSlider').value);"
  ...
  str += '<div class="listing">'
    ...
    + CookiStocker.calcCommission(stockerMinBrokers) + '</span>)</span>'
    + CookiStocker.note('stockerMinBrokers', false) + '</div>';

  var stockerCookiesPercent = Math.round((stockerCookiesThreshold || 0) * 100);
  var cbCookies =
      "var v = Math.round(l('cookiesPercentSlider').value);"
    + " stockerCookiesPercent = v; stockerCookiesThreshold = v / 100;"
  ...
  str += '<div class="listing">'
    ...
    + ' style="text-align:right !important; width:3ch !important; min-width:3ch !important; max-width:3ch !important;" inputmode="numeric" '
  ...
  str += '<div class="listing">'
    ...
    + '<input id="CS_loopFreq" type="text" size="5" value="' + loopSeconds + '" '
    + ' style="text-align:right !important; width:5ch !important; min-width:5ch !important; max-width:5ch !important;" inputmode="numeric" '
...
CookiStocker.updateAdditionalStatsVisibility = CookiStocker.updateAdditionalStatsVisibility;
...
CookiStocker.DataStats = function(id, value, dollars) {
  ...
  if (value > 0) {
    it.classList.add("green"); it.style.color = '';
  } else if (value < 0) {
    it.classList.remove("green"); it.classList.remove("yellow"); it.style.color = '#ff3b3b';
  }
};
...
// ====== SAVE / LOAD / RESET ======
// (unchanged from your version except safer CFG parsing and clamps)
...
CookiStocker.load = function(str) {
  let i = 0, j, k, m;
  if (typeof CookiStocker.Bank === 'undefined' || !str || !(stockList.Goods[0] && stockList.Goods[0].name !== undefined)) return false;
  ...
  for (j = 0; j < market.length; j++) {
    var tok = (spl[i++] || '');
    var nm; try { nm = decodeURIComponent(tok); } catch (e) { nm = tok; }
...
Game.registerMod('CookiStocker',{
  init: function () {
    ...
    (function waitCCSE(tries) {
      // Make sure the *function reference exists* before trying it
      if (typeof CCSE !== 'undefined'
  ...
  startStocking: function () {
    ...
    // Achievements + report timer + first stats paint
    if (typeof CCSE !== 'undefined' && CCSE && typeof CCSE.NewAchievement === 'function') {
    ...
    CookiStocker._loopTimer = setInterval(function() {
      ...
      if (CookiStocker.Bank.profit >= 100000000 && !Game.Achievements['Plasmic assets'].won) Game.Win('Plasmic assets');
      if (CookiStocker.Bank.profit >= 500000000 && !Game.Achievements['Bose-Einstein Condensed Assets'].won) Game.Win('Bose-Einstein Condensed Assets');
      ...
      if (!Game.OnAscend && (stockerAutoBuyMinimumBrokers || stockerAutoBuyAdditionalBrokers)) {
        ...
        let cost = CookiStocker.Bank.getBrokerPrice() * buyMoreBrokers;
        if (stockerAutoBuyAdditionalBrokers && buyMoreBrokers > 0 && cost < Game.cookies * 0.1) {
          Game.Spend(cost);
      ...
      for (let i = 0; i < market.length; i++) {
        ...
        let lastPrice = (stockList.Goods[i] ? stockList.Goods[i].currentPrice : market[i].val);
        let currentPrice = market[i].val;

        if (!stockList.Goods[i]) stockList.Goods[i] = {};
        ...
        let lmd = stockList.Goods[i].lastMode ?? md;
        ...
        if (stockList.Goods[i].lastDur != market[i].dur || ++(stockList.Goods[i].unchangedDur) > 1) {
          stockList.Goods[i].unchangedDur = 0; doUpdate = true;
        }
        if (Game.ObjectsById[i+2].amount == 0 && stockerConsoleAnnouncements && doUpdate && stockList.canBuy) {
          console.log(`${stockName} stock is inactive`); continue;
        }
        if (lmd == md && (stockList.Goods[i].stock && (md == 2 || md == 4) || !stockList.Goods[i].stock && (md == 1 || md == 3)))
        ...
        if (stockerConsoleAnnouncements && doUpdate && stockList.canBuy) {
          ...
          else console.log(`MODE CHANGE ${stockName} old=${lmd} [${modeDecoder[lmd]}] → new=${md} [${modeDecoder[md]}] at $${Beautify(currentPrice, 2)}`);
        }
        ...
        if (
          ...
          if (stockerTransactionNotifications)
            if (currentPrice >= 2) Game.Notify(`Buying ${stockName} ${new Date().toLocaleTimeString([], {hourCycle: 'h23', hour: '2-digit', minute: '2-digit'})}`,`Buying ${units} unit${(units > 1 ? 's' : '')}. The stock ${mode} at $${Beautify(market[i].prev, 2)} per unit and is in ${modeDecoder[md]} mode now.`,goodIcons[i],stockerFastNotifications*6);
            else Game.Notify(`Buying ${stockName} ${new Date().toLocaleTimeString([], {hourCycle: 'h23', hour: '2-digit', minute: '2-digit'})}`, `Buying ${units} unit${(units > 1 ? 's' : '')}. Price < $2; your buy is $${Beautify(market[i].prev, 2)}.`,goodIcons[i],stockerFastNotifications*6);
        ...
        // SELL
          ...
          if (stockerTransactionNotifications) Game.Notify(
            `Selling ${stockName} ${new Date().toLocaleTimeString([], {hourCycle: 'h23', hour: '2-digit', minute: '2-digit'})}`,
            `Selling ${stockList.Goods[i].stock} unit${(stockList.Goods[i].stock > 1 ? 's' : '')} at $${Beautify(market[i].val, 2)} for a ${strProfit} of $${Beautify(profit, 2)}.`,
            goodIcons[i],
            stockerFastNotifications*6
          );
          if (stockerConsoleAnnouncements) console.log(`=====$$$=== Selling ${stockName} at $${Beautify(market[i].val, 2)} for ${strProfit}$${Beautify(profit, 2)}.`);
        }
...
CookiStocker.Reports = function() {
  ...
  if (stockerActivityReport) {
    if ((stockList.Purchases + stockList.Sales) == 0) {
      Game.Notify(`CookiStocker report ${new Date().toLocaleTimeString([], {hourCycle: 'h23', hour: '2-digit', minute: '2-digit'})}`,
        `This session has been running for ${stockerTimeBeautifier(stockList.Uptime)}, but no good investment opportunities were detected!`,
        [1,33], stockerNotificationTime);
    } else {
      Game.Notify(`CookiStocker report ${new Date().toLocaleTimeString([], {hourCycle: 'h23', hour: '2-digit', minute: '2-digit'})}`,
        `Runtime ${stockerTimeBeautifier(stockList.Uptime)}; net profits $${Beautify(stockList.netProfits,0)}; displayed profits $${Beautify(stockList.Profits,0)} in ${Beautify(stockList.Purchases,0)} buy / ${Beautify(stockList.Sales,0)} sell.`,
        [1,33], stockerNotificationTime);
    }
  }
  ...
  if (stockerConsoleAnnouncements) {
    ...
    console.log(`Total profits = $${Beautify(totalProfits,2)} ${deltaTotalProfits ? ('  Δ=$'+Beautify(deltaTotalProfits,2)) : ''} ${totalTrades?('  trades='+totalTrades):''}`);
    console.log(`Profit per hour = $${Beautify(stockList.hourlyProfits,2)}; per day = $${Beautify(stockList.dailyProfits,2)}`);
    console.log(`That's ${Beautify(stockList.hourlyProfits * Game.cookiesPsRawHighest,2)} cookies/hr and ${Beautify(stockList.dailyProfits * Game.cookiesPsRawHighest,2)} cookies/day.`);
    if (stockerForceLoopUpdates) {
      console.log('Unadjusted (true) numbers:');
      stockList.hourlyProfits *= 60000 / stockerLoopFrequency;
      stockList.dailyProfits  *= 60000 / stockerLoopFrequency;
      console.log(`Per hour = $${Beautify(stockList.hourlyProfits,2)}; per diem = $${Beautify(stockList.dailyProfits,2)}`);
    }
    // zero delta bucket
    for (let j=0;j<6;j++) for (let k=0;k<6;k++) stockerModeProfits[j][k][1] = 0;
    console.log('------------------------------------------------------------------');
  }
};
